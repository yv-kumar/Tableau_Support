...
$getservice = Get-WmiObject -Class Win32_Service -Filter "StartName like '%$svcid%'"
}
...
try
{
$loadservice = Get-WmiObject -Class Win32_Service -Filter "Name = '$servicename'"
...
$restartservice = Get-Service -Name $servicename | Restart-Service
#Get-Service -Name $servicename | Restart-Service
}
catch { write-log ("ERROR: There is an error while restarting service *$servicename* on computer *$machinename* with error :_" + $_.Exception.Message)  }
} # If block end
...
if ($currentstatus -eq "Running"){ write-log ("INFO: Final Status of service *$servicename* on computer *$machinename* is *$currentstatus*") }
else { write-log ("ERROR: Attention Required! - Final Status of service *$servicename* on computer *$machinename* is *$currentstatus*") }
